...
Thumbnail-level clusters are obtained by HDBSCAN on a **10-D UMAP of the raw 1024-D CLS vectors** (`n_neighbors=15`, `min_dist=0`, `eom`, `min_cluster_size=20`, `min_samples=20`). There is no PCA step. A 2-D UMAP is used only for plots. The working run is `nopca-n15-mcs20-ms20` (33 clusters, 41.1% noise on seed 42), with two residual peels of leftover noise and a residual-CLS re-cluster after subtracting parent-cluster means. The operating residual cut is `nopca-n15-mcs20-ms20-residual-leaf` (leaf, `min_samples=10`); inherited eom is a two-cluster control and is not used. Publication-oriented methods and results: [cls_clustering.md](cls_clustering.md). Residual Stage 1 keep / skip list: [cls_residual_stage1.md](cls_residual_stage1.md). Commands and run IDs: [README-dev.md](https://github.com/lyashevska/thmbnails/blob/main/README-dev.md), [dinov3_runs.md](dinov3_runs.md).